...
type FocusAction =
  ...
  | {
      type: "START_SESSION";
      payload: { type: "quick" | "focus" | "short-break" | "long-break" };
    }
  | { type: "STOP_SESSION" }
  | { type: "TICK" }
  ...
  switch (action.type) {
    ...
    case "START_SESSION":
      ...
      const newSession: FocusSession = {
        ...
        type:
          action.payload.type === "short-break" ||
          action.payload.type === "long-break"
            ? "break"
            : action.payload.type,
        completed: false,
      };
  ...
  useEffect(() => {
    ...
    if (savedState) {
      try {
        ...
        console.log(
          "📥 FocusContext: Estado carregado do localStorage",
          parsedState
        );
    ...
    if (state.isActive && state.timeRemaining > 0) {
      ...
      if (state.settings.soundEnabled) {
        const audio = new Audio(
          "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDpatbEzHmvP/M4b1Fv/k0Lrz+z+O8wL/9Mz8="
        );
        audio
          .play()
          .catch((e) => console.log("🔇 Audio não pôde ser reproduzido:", e));
      }
    }
